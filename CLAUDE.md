...
독자는 PAA 키워드를 검색해서 이 글에 도착한다.
글을 다 읽었을 때 **"내 상황에서 뭘 어떻게 하면 되는지" 100% 답을 얻어야** 한다.
법령 정보를 나열하는 글이 아니라, 독자의 문제를 해결하는 글이다.

### 3대 금지

1. **정보 나열 금지**: "~이 있어요, ~도 있어요" 식으로 늘어놓지 않는다
2. **법령 번역 금지**: source-data 문장을 구어체로 바꾸기만 하면 안 된다. 독자 관점으로 재구성한다
3. **결론 미루기 금지**: 배경 설명부터 시작하지 않는다. 답부터 말한다

### 섹션 본문 작성법: 결론 → 조건 → 실행

각 섹션(3문단)은 반드시 이 순서를 따른다:

**문단 1 (결론부터)**: 독자가 가장 궁금한 것에 대한 답을 첫 문장에서 준다.
- ✅ "네, 입사일부터 자동으로 직장가입자가 돼요."
- ✅ "숙려기간은 자녀가 있으면 3개월, 없으면 1개월이에요."
- ❌ "국민건강보험제도는 사회보장제도의 일환으로..." (배경부터 시작)
- ❌ "이 제도에 대해 알아보겠습니다." (서론형)

**문단 2 (조건·예외·상황별 분기)**: "내 경우는?" 에 답한다.
- 상황별로 달라지는 내용을 구체적으로 분기해서 설명
- 표가 필요하면 `<table>` 사용
- "A인 경우에는 ~이고, B인 경우에는 ~이에요" 형태로 독자가 자기 상황을 찾을 수 있게

**문단 3 (실행 방법)**: "그래서 뭘 하면 되나?" 에 답한다.
- 어디서, 뭘 준비해서, 어떻게 하면 되는지 구체적으로
- 기한이 있으면 반드시 명시
- 온라인 신청 가능 여부, 필요 서류 등 실용 정보

### 자가검증 질문 (섹션 작성 후 반드시 확인)

섹션을 하나 쓸 때마다 아래 3가지를 자문한다:

1. **"이 소제목을 검색한 사람이 첫 문장만 읽어도 핵심 답을 알 수 있는가?"**
2. **"내 상황이 다른 경우(나이, 소득, 가족구성 등)에 대한 분기가 있는가?"**
3. **"읽고 나서 바로 행동할 수 있는 실행 정보(장소, 서류, 기한)가 있는가?"**

하나라도 "아니오"이면 문단을 다시 쓴다.

### 예시: Before → After

**Before (정보 나열형)**
```
국민건강보험은 질병이나 부상으로 인한 진료비 부담을 줄이기 위한 사회보장제도예요.
국민 누구나 가입할 수 있고, 직장가입자·지역가입자·피부양자 3종류가 있어요.
다만 의무가입이기 때문에 본인이 탈퇴할 수는 없어요.
```

**After (문제해결형)**
```
회사에 입사하면 별도 신청 없이 자동으로 직장가입자가 돼요.
사업주가 입사일로부터 14일 이내에 건강보험공단에 자격취득 신고를 하게 되어 있어서,
본인이 따로 할 일은 없어요.

다만 1개월 미만 일용근로자, 시간제 공무원 등은 직장가입자가 아니라 지역가입자로
분류돼요. 본인이 프리랜서이거나 특수고용직이라면 지역가입자에 해당하는지
건강보험공단(1577-1000)에 확인하는 게 좋아요.

직장가입자 확인은 건강보험공단 홈페이지(nhis.or.kr)에서 공인인증서로 로그인하면
자격득실확인서를 바로 출력할 수 있어요. 팩스나 우편 신청도 가능해요.
```
