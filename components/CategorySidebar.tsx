      ...
      <div className="sticky top-24">
        <nav className="rounded-xl border border-gray-200 bg-white p-4">
                ...
                <li key={spoke.slug}>
                  <Link
                    ...
                    className={`group flex items-center gap-2 rounded-lg px-3 py-2 text-sm transition-colors ${
                      isCurrent
                        ? "bg-gov-50 text-gov-700 font-bold border-l-2 border-gov-500"
                        : "text-gray-600 hover:bg-gray-50 hover:text-gov-600"
                    }`}
                  >
                    <ChevronRight
                      className={`h-3.5 w-3.5 shrink-0 ${
                        isCurrent ? "text-gov-500" : "text-gray-300 group-hover:text-gov-400"
                      }`}
                    />
          ...
          <div className="mt-3 pt-3 border-t border-gray-100 space-y-1">
            <Link
              href={`/${categorySlug}`}
              className="flex items-center gap-2 px-3 py-2 text-sm text-gray-500 hover:text-gov-600 transition-colors"
            >
